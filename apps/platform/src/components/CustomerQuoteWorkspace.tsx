...
import { realSendCustomerPortalMessage, realSubmitCustomerPortalDecision,realDecidePreviewProof,realGetCustomerFinalDeliverableDownload } from '@/lib/real-api-client.client';
...
export function CustomerQuoteWorkspace({ initialDetail }: { initialDetail: CustomerPortalRequestDetail }) {
  ...
  async function downloadFinal(finalDeliverableId:string){if(downloadingId)return;setDownloadingId(finalDeliverableId);setError(null);try{const result=await realGetCustomerFinalDeliverableDownload(initialDetail.request.requestId,finalDeliverableId);window.location.assign(result.downloadUrl);}catch{setError('The secure download link could not be prepared. Please try again.');}finally{setDownloadingId(null);}}
