...
/**
 * One row of a live reports list (PHX-REPORTS-001, vercel-supabase-preview
 * mode only — mirrors the `reports` table joined with its `report_templates`
 * row, its requesting user, and its optional asset). Exact same
 * architectural pattern as BackendPassport/BackendCertification: there is
 * no real-dev / production-auth counterpart yet — apps/backend/src/routes/
 * reports.ts is still a PHX-BACKEND-001 stub (every route 501s), so this
 * type is only ever populated by previewGetReports() in
 * preview-api-client.server.ts.
 *
 * PHX-REPORTS-001 note on field naming vs. the task brief's suggested
 * shape: the brief suggested `assessmentId` / `assessmentTitle` /
 * `generatedByUserId` / `generatedByDisplayName` / `reportType` fields.
 * Per @phoenix/core's `Report` contract (packages/core/src/contracts/
 * report.ts) and the actual `reports` table
 * (apps/backend/db/migrations/0001_initial_schema.sql), none of those
 * columns exist:
 *   - `reports` has no `assessment_id` column at all (a report is scoped
 *     to an optional `asset_id`, not an assessment — a Workspace-scope
 *     report may have no asset at all). Following the brief's own
 *     "only include fields supported by the real schema; do not fabricate
 *     missing values" instruction, `assessmentId`/`assessmentTitle` are
 *     omitted rather than invented.
 *   - The column actually tracking who a report is for is
 *     `requested_by_user_id` (a human requests a report; nothing in this
 *     schema tracks a separate "generated by" actor — generation is an
 *     automated job, not attributed to a user). `requestedByUserId` /
 *     `requestedByDisplayName` are used instead of the brief's
 *     `generatedByUserId` / `generatedByDisplayName`.
 *   - There is no `reportType` column; `templateName` (denormalized from
 *     `report_templates.name` via `template_id`) is the closest real
 *     equivalent and is included instead.
 * `name` (the report's own denormalized display name, per the Report
 * contract's doc comment: "Denormalized template name at generation time,
 * for display even if the template later changes") is included as-is —
 * this is a stored column, not a derived one.
 */
export interface BackendReport {
  id: string;
  workspaceId: string;
  templateId: string;
  /** Denormalized from report_templates.name via template_id. Closest real equivalent to the brief's "reportType". */
  templateName: string;
  /** The report's own denormalized display name (reports.name) — see interface doc comment. */
  name: string;
  /** ReportStatus: 'Requested' | 'Generating' | 'Available' | 'Expired' | 'Failed'. */
  status: string;
  /** Null for a Workspace-scope or CertificationPortfolio-scope report — reports.asset_id is nullable. */
  assetId: string | null;
  /** Null whenever assetId is null (no asset to join against). */
